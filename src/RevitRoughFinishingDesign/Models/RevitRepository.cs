...
internal class RevitRepository {
    ...
    public IList<GraphicsStyle> GetAllLineStyles() {
        _lineStyles ??= new FilteredElementCollector(Document)
    ...
    public IList<WallType> GetWallTypesInsideRoomsOnActiveView() {
        var wallIds = GetWallsIds();
        var roomsOnActiveView = GetRoomsOnActiveView();
        var wallTypes = new HashSet<ElementId>();

        foreach(var room in roomsOnActiveView) {
            foreach(var wallId in wallIds) {
                var wallElement = Document.GetElement(wallId) as Wall;
                var wallSolid = wallElement.GetSolids().First();
                var wallOrigin = wallSolid.ComputeCentroid();
                if(room.IsPointInRoom(wallOrigin)) {
                    _ = wallTypes.Add(wallElement.WallType.Id);
                }
            }
        }
    ...
    public ICollection<ElementId> GetWallsIds() {
        ...
        _wallsIds ??= new FilteredElementCollector(Document, ActiveView.Id)
            .WhereElementIsNotElementType()
            .OfClass(typeof(Wall))
    ...
    public IList<IList<BoundarySegment>> GetBoundarySegments(Room room) {
        return room is null ? throw new ArgumentNullException(nameof(room)) : room.GetBoundarySegments(_spatialElementBoundaryOptions);
    }
    ...
    public double GetVerticalPointFromActiveView() {
        if(_activeViewZPoint != -100000) {
            ...
            var viewPlan = ActiveView as ViewPlan;
    ...
    public Curve TransformCurveToExactZ(Curve curve, double exactZ) {
        ...
        var newStart = new XYZ(start.X, start.Y, exactZ);
        var newEnd = new XYZ(end.X, end.Y, exactZ);
    ...
    public RoomBorder GetClosestCurveFromCurveList(Curve currentCurve, IList<RoomBorder> roomBorders) {
        var centerOfCurrentCurve = currentCurve.Evaluate(0.5, true);
        double minDistance = double.PositiveInfinity;
        RoomBorder closestBorder = null;
        var currentLine = currentCurve as Line;
        var currentDirection = currentLine.Direction.Normalize();
        foreach(var roomBorder in roomBorders) {
            var roomBorderLine = roomBorder.Curve as Line;
            var roomBorderDirection = roomBorderLine.Direction.Normalize();
            //if(!currentDirection.IsAlmostEqualTo(roomBorderDirection)) {
            //    continue;
            //}
            if(AreVectorsAligned(currentDirection, roomBorderDirection)) {
                var closestPoint = roomBorder.Curve.Project(centerOfCurrentCurve).XYZPoint;
    ...
    public void CreateTestModelLine(Line geomLine) {
        var dir = geomLine.Direction.Normalize();
        double x = dir.X;
        double y = dir.Y;
        double z = dir.Z;

        var origin = geomLine.Origin;
        var normal = new XYZ(z - y, x - z, y - x);
        var plane = Plane.CreateByNormalAndOrigin(normal, origin);
        var sketch = SketchPlane.Create(Document, plane);
        _ = Document.Create.NewModelCurve(geomLine, sketch);
    }
