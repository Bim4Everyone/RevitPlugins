...
internal class MainViewModel : BaseViewModel {
    ...
    public ObservableCollection<LineStyleViewModel> LineStyles {
        get => _lineStyles;
        set => RaiseAndSetIfChanged(ref _lineStyles, value);
    }

    private ObservableCollection<PairViewModel> _items = [];
    ...
    private bool CanAcceptView() {
        if(!double.TryParse(LineOffset, out _)) {
    ...
    private void LoadConfig() {
        ...
        var pairModels = setting?.PairModels ?? [];
    ...
    private ObservableCollection<PairViewModel> GetRevitTypesData(
        ...
        var items = new ObservableCollection<PairViewModel>(
            allWallTypes.Select(wt => new PairViewModel(wt, noneStyle)));

        foreach(var item in items) {
            var pairModel = pairModels.FirstOrDefault(pm => pm.WallTypeId == item.WallTypeVM.WallTypeId);
            if(pairModel != null) {
                var lineStyle = allLineStyles
