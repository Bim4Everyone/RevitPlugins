...
internal class DesignTypeAnalyzer {
    ...
    private readonly List<string> _paramsForFormElements = ["ФОП_ТИП_Армирование"];

    private readonly List<string> _paramsForRebars = [
        "мод_ФОП_Диаметр",
        "обр_ФОП_Длина",
    ...
    private readonly List<string> _paramsForSysRebars = [
        "Полная длина стержня",
        "Количество"
    ];

    private readonly List<string> _paramsForIfcRebars = ["обр_ФОП_Количество"];
