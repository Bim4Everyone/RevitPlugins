...
internal class DesignTypeVM : BaseViewModel {
    ...
    /// </summary>
    public List<FormworkElement> Formworks { get; set; } = [];

    /// <summary>
    /// Арматурные элементы данного типа конструкции
    /// </summary>
    public List<RebarElement> Rebars { get; set; } = [];
    ...
    public double FormworkVolume {
        get => _formworkVolume;
        set => RaiseAndSetIfChanged(ref _formworkVolume, value);
    }
    ...
    public double RebarMass {
        get => _rebarMass;
        set => RaiseAndSetIfChanged(ref _rebarMass, value);
    }
    ...
    public string RebarCoef {
        get => _rebarCoef;
        set => RaiseAndSetIfChanged(ref _rebarCoef, value);
    }
