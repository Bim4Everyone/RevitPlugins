...
public class RevitKrParameterClassifierCommand : BasePluginCommand {
    /// <summary>
    ...
    protected override void Execute(UIApplication uiApplication) {
        ...
        kernel.BindMainWindow<KrParameterClassifierVM, KrParameterClassifierV>();
        ...
        kernel.UseWpfUIMessageBox<KrParameterClassifierVM>();

        // Сервис открытия диалогового окна для чтения файла Классификатора
        kernel.UseWpfOpenFileDialog<KrParameterClassifierVM>(
            filter: "Excel files (*.xlsx;*.xls)|*.xlsx;*.xls|All files (*.*)|*.*");

        // Вызывает стандартное уведомление
        Notification(kernel.Get<KrParameterClassifierV>());
    }
}
