...
/// <summary>
/// Делоновская триангуляция инкрементальным алгоритмом Боуэра-Уотсона.
/// </summary>
/// <remarks>
/// Сложность: O(n^2) в худшем случае без специальной структуры соседства,
/// что соответствует требованию плагина (до ~500 пилонов + 2-3 тыс. точек со стен).
/// Точки супер-треугольника сохраняются в результирующей триангуляции;
/// потребитель отфильтровывает треугольники, ссылающиеся на индексы супер-точек,
/// либо использует их (в нашем случае — для построения "висящих" ячеек Вороного на границе).
/// </remarks>
internal sealed class BowyerWatsonDelaunay {
    ...
    /// <summary>
    /// Индексы трех "бесконечных" вершин супер-треугольника в массиве <see cref="Points"/>.
    /// </summary>
    ...
    /// <summary>
    /// Триангулирует множество точек. Возвращает индексы исходных точек в <see cref="Points"/>
    /// (после супер-треугольника), сохраняя порядок входа.
    /// </summary>
    public int[] Triangulate(IReadOnlyList<Point2D> sites, BoundingBox2D bounds, double margin) {
    ...
    private void BuildSuperTriangle(BoundingBox2D bounds, double margin) {
        var center = bounds.Center;
        double diag = Math.Max(bounds.Diagonal, 1.0) + margin;
        double size = diag * 50;
        var a = new Point2D(center.X - size, center.Y - size);
        var b = new Point2D(center.X + size, center.Y - size);
        var c = new Point2D(center.X, center.Y + size * 2);
    ...
    public readonly struct Triangle {
        public Triangle(int v0, int v1, int v2, IReadOnlyList<Point2D> points) {
            V0 = v0;
            V1 = v1;
            V2 = v2;
            var a = points[v0];
        ...
        public bool HasVertex(int v) => V0 == v || V1 == v || V2 == v;

        public static Point2D ComputeCircumcenter(Point2D a, Point2D b, Point2D c) {
            double ax = a.X, ay = a.Y;
            double bx = b.X, by = b.Y;
            double cx = c.X, cy = c.Y;
            double d = 2.0 * ((ax * (by - cy)) + (bx * (cy - ay)) + (cx * (ay - by)));
            if(Math.Abs(d) < GeometryTolerance.Model) {
                return new Point2D((ax + bx + cx) / 3.0, (ay + by + cy) / 3.0);
            }
