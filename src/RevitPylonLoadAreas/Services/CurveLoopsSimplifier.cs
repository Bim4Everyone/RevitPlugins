...
internal class CurveLoopsSimplifier {
    /// <summary>
    /// Возвращает коллекцию контуров грани, в котором каждый контур упрощён
    /// </summary>
    /// <param name="face">Грань</param>
    /// <returns>Коллекция новых контуров с объединенными линиями</returns>
    public IList<CurveLoop> GetEdgesAsSimplifiedCurveLoops(PlanarFace face) {
        return face.GetEdgesAsCurveLoops().Select(Simplify).ToArray();
    }
    ...
    private bool IsContinuation(Curve first, Curve second) {
        return first is not null
               && second is not null
               && (first is Line firstLine)
               && (second is Line secondLine)
               && firstLine.Direction.IsAlmostEqualTo(secondLine.Direction)
    ...
    private Curve CombineCurves(Curve curveFirst, Curve curveSecond) {
        return curveFirst is null
            ? throw new ArgumentNullException(nameof(curveFirst))
            : curveSecond is null
                ? throw new ArgumentNullException(nameof(curveSecond))
                : (Curve) Line.CreateBound(curveFirst.GetEndPoint(0), curveSecond.GetEndPoint(1));
    }
}
