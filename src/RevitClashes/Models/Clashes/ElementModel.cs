...
internal class ElementModel : IEquatable<ElementModel> {
    ...
    public Element GetElement(IEnumerable<DocInfo> docInfos) {
        return docInfos is null
            ? throw new ArgumentNullException(nameof(docInfos))
            : docInfos.Any(item => item is null)
            ? throw new ArgumentNullException(nameof(docInfos))
            : (GetDocInfo(docInfos)?.Doc.GetElement(Id));
    }
    ...
    public bool Equals(ElementModel other) {
        return other is not null && (ReferenceEquals(this, other) || (other != null
            && Id == other.Id
            && Name == other.Name
