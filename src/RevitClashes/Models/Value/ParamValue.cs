...
internal abstract class ParamValue : IComparable<ParamValue>, IEquatable<ParamValue> {
    ...
    private protected ElementId GetParamId(Document doc, RevitParam revitParam) {
        var paramId = ElementId.InvalidElementId;
        paramId = revitParam is SystemParam systemParam
            ? new ElementId(systemParam.SystemParamId)
            : revitParam.GetRevitParamElement(doc)?.Id ?? ElementId.InvalidElementId;
        return paramId;
    }
}
