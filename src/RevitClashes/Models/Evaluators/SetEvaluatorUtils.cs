...
internal class SetEvaluatorUtils {
    ...
    public static IRevitLogicalFilterCreator GetRevitLogicalFilterCreator(SetEvaluators evaluator) {
        _evaluatorDictionary.TryGetValue(evaluator, out var result);
        return result;
    }

    public static IRevitLogicalFilterCreator GetInvertedRevitLogicalFilterCreator(SetEvaluators evaluator) {
        _invertedEvaluatorDictionary.TryGetValue(evaluator, out var result);
        return result;
    }

    private static readonly Dictionary<SetEvaluators, IRevitLogicalFilterCreator> _evaluatorDictionary = new() {
        {SetEvaluators.And, new RevitLogicalAndFilterCreator() },
        {SetEvaluators.Or, new RevitLogicalOrFilterCreator() },
    };

    private static readonly Dictionary<SetEvaluators, IRevitLogicalFilterCreator> _invertedEvaluatorDictionary = new() {
