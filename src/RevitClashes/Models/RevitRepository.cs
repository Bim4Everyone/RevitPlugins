...
internal class RevitRepository {
    ...
    private static readonly HashSet<string> _endings = ["_отсоединено", "_detached"];
    ...
    public ParameterFilterProvider ParameterFilterProvider { get; }
    ...
    public static string LocalProfilePath => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "dosymep");

    public static List<BuiltInParameter> BaseLevelParameters => [
    ...
    public View3D GetClashView() {
        ...
        if(view == null) {
            using var t = Doc.StartTransaction("Создание 3D-вида");
    ...
    public static string GetDocumentName(string fileName) {
        foreach(string ending in _endings) {
    ...
    public string GetObjectName() {
        return Doc.Title.Split('_').FirstOrDefault();
    }

    public Element GetElement(string fileName, ElementId id) {
        var doc = fileName == null
            ? Doc
            : (DocInfos.FirstOrDefault(item => item.Name.Equals(GetDocumentName(fileName), StringComparison.CurrentCultureIgnoreCase))?.Doc);
        var elementId = id;
        return doc == null || elementId.IsNull() ? null : doc.GetElement(elementId);
    }
    ...
    public bool IsParentLink(RevitLinkInstance link) {
        if(link.GetTypeId().IsNotNull()) {
            ...
            return type is RevitLinkType linkType && !linkType.IsNestedLink;
        }
        return false;
    }
    ...
    public void InitializeDocInfos() {
        DocInfos = GetRevitLinkInstances()
           ...
           .ToList();
        DocInfos.Add(new DocInfo(GetDocumentName(), Doc, Transform.Identity));
    }

    public List<Category> GetCategories() {
        return ParameterFilterProvider.GetAllModelCategories(Doc, _view)
    ...
    public List<ParameterValueProvider> GetParameters(Document doc, IEnumerable<Category> categories) {
        return categories
            ...
            .SelectMany(FilterParamProviders)
            .ToList();
    }
    ...
    private ICollection<ParameterFilterElement> GetHighlightFilters(
        ...
        List<ParameterFilterElement> parameterFilters = [];
        try {
            parameterFilters.Add(ParameterFilterProvider.CreateParameterFilter(
                Doc,
        ...
        parameterFilters.Add(ParameterFilterProvider.GetExceptCategoriesFilter(
            Doc,
            view,
            categoriesToShow,
    ...
    private void HighlightFilter(
        ...
        using var t = Doc.StartTransaction("Выделение элементов коллизии");
    ...
    public ICollection<ElementId> GetLineCategoriesToHide() {
        return new ElementId[] {
            new(BuiltInCategory.OST_MEPSpaceSeparationLines),
            new(BuiltInCategory.OST_RoomSeparationLines),
            new(BuiltInCategory.OST_Lines)
        };
    }
    ...
    public static Level GetLevel(Element element) {
        ...
        return element.LevelId.IsNotNull() ? element.Document.GetElement(element.LevelId) as Level : null;
    }
    ...
    private ICollection<ElementId> GetElementsToSelect(IEnumerable<Element> elements, View3D view = null) {
        ...
        var elementsFromThisDoc = elements
            .Where(item => item.IsFromDocument(Doc))
            .Select(item => item.Id)
            .ToList();
    ...
    public ICollection<BoundingBoxXYZ> GetBoundingBoxes(IEnumerable<ElementModel> elements) {
        return elements
            .Select(item => new {
                ...
                item.GetDocInfo(DocInfos)?.Transform
            })
    ...
    private ParameterValueProvider GetParam(Document doc, Category category, ElementId elementId) {
        ...
        return elementId.IsSystemId()
            ? new ParameterValueProvider(this, revitParam, $"{revitParam.Name} ({category.Name})")
            : new ParameterValueProvider(this, revitParam);
    }
    ...
    public void SetSectionBox(BoundingBoxXYZ bb, View3D view, double additionalSize) {
        ...
        using var t = Doc.StartTransaction("Подрезка");
