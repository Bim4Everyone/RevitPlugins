...
internal class ParameterFilterProvider {
    ...
    public ParameterFilterElement CreateFilter(Document document, string filterName, BuiltInCategory category) {
        return document is null
            ...
            : CreateFilter(document, filterName, new BuiltInCategory[] { category });
    }
    ...
    private ElementFilter GetHighlightElementFilter(Element element, ParameterFilterElement checker) {
        ...
        if(element.HasElementType()) {
            var typeName = BuiltInParameter.ALL_MODEL_TYPE_NAME;
            ...
            var typeNameNotEqualsRule = _notEqualsVisister.Create(new ElementId(typeName), elType);
    ...
    private ElementFilter GetSelectElementFilter(Element element, ParameterFilterElement checker) {
        ...
        if(element.HasElementType()) {
            ...
            var typeNameEqualsRule = _equalsVisiter.Create(new ElementId(typeName), elType);
        ...
        foreach(var param in doubleParameters) {
            ...
            var ruleStart = _greaterOrEqualVisister.Create(param.Id, value - delta);
            var ruleEnd = _lessOrEqualVisister.Create(param.Id, value + delta);
