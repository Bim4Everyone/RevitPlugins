...
internal class AdvancedRayAreaExtrusionBuilder : IBuildAreaExtrusion {
    ...
    private readonly ReferenceIntersector _referenceIntersector;


    public AdvancedRayAreaExtrusionBuilder(
        ...
        _view3D = _revitRepository.ActiveUIDocument.ActiveGraphicalView as View3D;
        _referenceIntersector = GetIntersector();
    }
    ...
    public ReferenceIntersector GetIntersector() {
        ...
        foreach(string krName in _buildCoordVolumesSetting.TypeSlabs) {
        ...
        var intersector = new ReferenceIntersector(logicalAndFilter, FindReferenceTarget.Element, _view3D) {
    ...
    public void BuildAreaExtrusion(RevitArea area) {
        ...
        //MessageBox.Show(slabsByLevel.Count.ToString(), "группировка плит по уровню для промежуточных сегментов");
        ...
        var allSlabId = _allSlabs
            .Select(slab => slab.Floor.Id);
        ...
        foreach(var polygon in polygons) {
            ...
            var insideSlabsIds = _referenceIntersector.Find(polygon.Center, new XYZ(0, 0, 10))
                .Select(rwc => rwc.GetReference().LinkedElementId)
                .ToList();

            var insideSlabs = _allSlabs
                .Where(slab => insideSlabsIds.Contains(slab.Floor.Id))
                .ToList();
                    ...
                    for(int k = idxStart + 1; k <= idxEnd; k++) {
                        ...
                        foreach(var slab in slabsAtLevel) {
                            ...
                            columns.Add(new Column() {
                                ...
                                StartZ = segStartZ,
                                EndZ = segEndZ,
                                StartLevel = (k == idxStart + 1) ? startLevel : "Undefined",
