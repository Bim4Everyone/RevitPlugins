...
internal class CurveDividerService(SystemPluginConfig systemPluginConfig) {
    ...
    public List<Curve> SplitCurvesAtIntersections(List<Curve> curves) {
        ...
        var index = new CurveSpatialIndex(curves, _maxLenCurve);

        for (int i = 0; i < n; i++) {
            var c1 = curves[i];
            var b1 = index.Boxes[i];
            ...
            foreach (int j in index.Query(b1)) {
                ...
                for (int k = 0; k < ira.Size; k++) {
                    ...
                    bool inside1 = Math.Abs(p1 - starts[i]) > _tolerance && Math.Abs(p1 - ends[i]) > _tolerance;
                    bool inside2 = Math.Abs(p2 - starts[j]) > _tolerance && Math.Abs(p2 - ends[j]) > _tolerance;

                    if (inside1) {
                        splitParameters[i].Add(p1);
                    }
                    if (inside2) {
                        splitParameters[j].Add(p2);
                    }
                }
            }
        }
    ...
    private static bool AreCollinearLines(Curve l1, Curve l2, double tol) {
        ...
        var d1 = line1.Direction.Normalize();
        var d2 = line2.Direction.Normalize();
        ...
        var v = line2.GetEndPoint(0) - line1.GetEndPoint(0);
