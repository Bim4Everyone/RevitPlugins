...
internal class FreeEndsJoinService(SystemPluginConfig systemPluginConfig): IFreeEndsJoinService {
    ...
    private List<FreeEnd> CollectFreeEnds(List<Curve> curves, CurveSpatialIndex index) {
        ...
        for (int i = 0; i < curves.Count; i++) {
            if (curves[i] is not Line line) {
                continue;
            }

            var p0 = line.GetEndPoint(0);
            if (!IsEndpointConnectedIndexed(p0, i, curves, index, _tol)) {
                var dirOut0 = (line.GetEndPoint(0) - line.GetEndPoint(1)).Normalize();
            ...
            var dirOut1 = (line.GetEndPoint(1) - line.GetEndPoint(0)).Normalize();
    ...
    private List<PairCandidate> BuildPairCandidatesWithScore(List<FreeEnd> freeEnds, double maxJoin, double angleWeight) {
        ...
        for (int a = 0; a < freeEnds.Count; a++) {
            ...
            foreach (int b in QueryNearby(grid, fa.Point, maxJoin)) {
                ...
                if (fa.CurveIndex == fb.CurveIndex) {
                    continue;
                }
                ...
                var ab = (fb.Point - fa.Point).Normalize();
                var ba = (fa.Point - fb.Point).Normalize();
    ...
    private bool IsEndpointConnectedIndexed(XYZ point, int ownerIndex, List<Curve> curves, CurveSpatialIndex index, double tol) {
    ...
    private sealed class FreeEnd(int curveIndex, int endIndex, XYZ point, XYZ outDirection) {
        public int CurveIndex { get; } = curveIndex;
        public int EndIndex { get; } = endIndex;
        public XYZ Point { get; } = point;
        public XYZ OutDirection { get; } = outDirection; // направление "наружу" от свободного конца
    }

    private readonly struct PairCandidate(int a, int b, double distance, double score) {
        public int A { get; } = a;
        public int B { get; } = b;
