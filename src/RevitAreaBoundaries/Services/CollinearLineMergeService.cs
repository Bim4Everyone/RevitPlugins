...
internal class CollinearLineMergeService(SystemPluginConfig config): ICollinearLineMergeService{
    ...
    private List<int> CollectCollinearComponent(int start, List<Line> lines, CurveSpatialIndex index, bool[] visited) {
        ...
        while (q.Count > 0) {
            ...
            foreach (int j in index.Query(probe)) {
                ...
                var lj = lines[j];
                if (!AreCollinear(li, lj, _tol)) {
                    continue;
                }
                if (!AreConnectedOrOverlapping(li, lj, _tol)) {
                    continue;
                }
    ...
    private Line MergeGroup(List<Line> lines, List<int> idxs) {
        ...
        var baseLine = lines[idxs[0]];
        var origin = baseLine.GetEndPoint(0);
        var dir = (baseLine.GetEndPoint(1) - baseLine.GetEndPoint(0)).Normalize();
        ...
        foreach (int idx in idxs) {
            var l = lines[idx];
            var p0 = l.GetEndPoint(0);
            var p1 = l.GetEndPoint(1);
        ...
        var a = origin + dir * minT;
        var b = origin + dir * maxT;

        return a.DistanceTo(b) <= _tol 
            ? Line.CreateBound(baseLine.GetEndPoint(0), baseLine.GetEndPoint(1)) 
    ...
    private static bool AreCollinear(Line l1, Line l2, double tol) {
        var d1 = (l1.GetEndPoint(1) - l1.GetEndPoint(0)).Normalize();
        var d2 = (l2.GetEndPoint(1) - l2.GetEndPoint(0)).Normalize();
        ...
        var v = l2.GetEndPoint(0) - l1.GetEndPoint(0);
        return d1.CrossProduct(v).GetLength() <= tol;
    }

    private static bool AreConnectedOrOverlapping(Line l1, Line l2, double tol) {
        var a0 = l1.GetEndPoint(0);
        var a1 = l1.GetEndPoint(1);
        var b0 = l2.GetEndPoint(0);
        var b1 = l2.GetEndPoint(1);
