...
public class CellsBoundaryService (SystemPluginConfig systemPluginConfig){
    ...
    private readonly double _stepFine = UnitUtils.ConvertToInternalUnits(
        systemPluginConfig.DefaultCellsFineStepMm, UnitTypeId.Millimeters);

    public HashSet<CellSquare> GetCoarseCells(List<XYZ> squareVertices, List<Curve> curves) {
        ...
        var bl = GetAlignedPoint(new XYZ(minX, minY, 0), _stepCoarse);
        var br = GetAlignedPoint(new XYZ(maxX, minY, 0), _stepCoarse);
        var tr = GetAlignedPoint(new XYZ(maxX, maxY, 0), _stepCoarse);
        var tl = GetAlignedPoint(new XYZ(minX, maxY, 0), _stepCoarse);

        var index = new CurveSpatialIndex(curves, cellSize: _stepCoarse);

        // 4 очереди (round-robin)
        var queues = new[] { new Queue<XYZ>(), new Queue<XYZ>(), new Queue<XYZ>(), new Queue<XYZ>() };
        ...
        foreach(var p in GetPerimeterSeeds(minX, minY, maxX, maxY, _stepCoarse)) {
            var ap = AlignToGridFloor(p, _stepCoarse);

            // разложим по очередям round-robin, чтобы нагрузка была равномерной
            int bucket = Math.Abs(GetKey(ap, _stepCoarse).GetHashCode()) % 4;
        ...
        while(AnyQueueNotEmpty()) {
            ...
            string currentKey = GetKey(current, _stepCoarse);
            if(!visited.Add(currentKey)) {
                continue;
            }

            foreach(var next in GetNeighbours(current, _stepCoarse)) {
                if(next.X < minX || next.X > maxX || next.Y < minY || next.Y > maxY) {
                    continue;
                }

                string nextKey = GetKey(next, _stepCoarse);
                ...
                if(hit) {
                    var cell = hitPoint != null
                        ? MakeCellFromPoint(hitPoint, _stepCoarse)
                        : MakeCellFromEdge(current, next, _stepCoarse);
                    ...
                    if(hitCellKeys.Add(cellKey)) {
                        // Добавление кривых, находящихся внутри ячейки
                        cell.Curves = GetCurvesInsideCellFast(cell, index);
                        hitCells.Add(cell);
                    }
                    continue;
                }
    ...
    private static XYZ GetAlignedPoint(XYZ startPoint, double step) {
        ...
        return new XYZ(start.X + nudge, start.Y + nudge, 0);
    }
    ...
    private bool FindNearestHitCurveFast(XYZ from, XYZ to, CurveSpatialIndex index, out Curve hitCurve) {
        ...
        var probeBox = new BBox(
            probeBox0.MinX - _tolerance,
            probeBox0.MinY - _tolerance,
            probeBox0.MaxX + _tolerance,
            probeBox0.MaxY + _tolerance);
    ...
    private bool FindHitFast(XYZ from, XYZ to, CurveSpatialIndex index, out XYZ hitPoint) {
        ...
        foreach(int id in index.Query(probeBox)) {
            ...
            if(TryGetIntersectionPoint(probe, c, out var ip)) {
            ...
            // 3) Fallback (если хочешь оставить твой старый метод касания)
    ...
    private double PointSegmentDistance2D(XYZ p, XYZ a, XYZ b, out XYZ closest) {
        var ab = b - a;
    ...
    private double SegmentSegmentDistance2D(XYZ a, XYZ b, XYZ c, XYZ d, out XYZ closestOnAb) {
    ...
    private bool SegmentsIntersect2D(XYZ a, XYZ b, XYZ c, XYZ d, out XYZ ip) {
        ip = null;
        ...
        if(!(t >= -_tolerance) || !(t <= 1 + _tolerance) || !(u >= -_tolerance) || !(u <= 1 + _tolerance)) {
            return false;
        }
    ...
    private bool SegmentBlocks(Curve probe, Curve boundary) {
        ...
        // Fallback на случай численных погрешностей: "почти касание"
        var a = probe.GetEndPoint(0);
        var b = probe.GetEndPoint(1);
        var mid = new XYZ((a.X + b.X) * 0.5, (a.Y + b.Y) * 0.5, (a.Z + b.Z) * 0.5);

        if(boundary.Distance(a) < _tolerance) {
            return true;
        }
        if(boundary.Distance(mid) < _tolerance) {
            return true;
        }
        
        return boundary.Distance(b) < _tolerance;
    }
    ...
    private static bool IsPointInsideCell(XYZ point, CellSquare cell) {
    ...
    private static bool IsCurveInsideCell(Curve curve, CellSquare cell) {
    ...
    private List<Curve> GetCurvesInsideCellFast(CellSquare cell, CurveSpatialIndex index) {
        // bbox ячейки
        double minX = cell.BottomLeft.X;
        double minY = cell.BottomLeft.Y;
        double maxX = cell.TopRight.X;
        ...
        // Чуть расширим, чтобы не терять касания из‑за погрешностей
        var box = new BBox(minX - _tolerance, minY - _tolerance, maxX + _tolerance, maxY + _tolerance);

        var result = new List<Curve>();

        foreach (int id in index.Query(box)) {
            // дополнительная проверка — индекс может вернуть лишнее
            if (!box.Intersects(index.Boxes[id]))
                continue;

            var c = index.Curves[id];

            // минимальная логика как раньше (по концам)
            if (IsCurveInsideCell(c, cell))
                result.Add(c);
        }
        return result;
    }
