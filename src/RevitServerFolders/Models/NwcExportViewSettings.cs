namespace RevitServerFolders.Models;
/// <summary>
/// Настройки вида для экспорта в NWC
/// </summary>
internal class NwcExportViewSettings {
    /// <summary>
    /// Путь к файлу, в котором находится шаблон вида для экспорта в NWC
    /// </summary>
    public string RvtFilePath { get; set; }

    /// <summary>
    /// Название шаблона вида, который надо применить к экспортируемому в NWC виду
    /// </summary>
    public string ViewTemplateName { get; set; }

    /// <summary>
    /// Рабочие наборы, в названиях которых содержится подстрока из коллекции, не будут открыты
    /// при открытии экспортируемого документа и должны быть скрыты на экспортируемом виде
    /// </summary>
    public string[] WorksetHideTemplates { get; set; } = ["вспомогательные", "скрытые", "скрыть"];

    /// <summary>
    /// Подстроки описаний предупреждений, элементы которых удалять нельзя:
    /// их удаление приводит к отмене открытия документа.
    /// Настройка доступна только через json файл конфигурации.
    /// </summary>
    public string[] WarningKeepElementsTemplates { get; set; }
        = ["помещение", "пространство", "room", "space", "окружен", "enclosed", "area", "зона"];

    /// <summary>
    /// Подстроки описаний предупреждений, элементы которых надо удалять
    /// (битые размеры, потерянные ссылки и т.п.).
    /// Настройка доступна только через json файл конфигурации.
    /// </summary>
    public string[] WarningDeleteElementsTemplates { get; set; }
        = ["размер", "dimension", "радиус", "radius", "ссылк", "reference", "spot"];
}
