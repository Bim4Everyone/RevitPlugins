...
internal class FamilyViewModel : BaseViewModel {
    ...
    public FamilyViewModel(RevitRepository revitRepository) {
        ...
        Name = _revitRepository.DocumentTitle;
    ...
    public FamilyParamViewModel SelectedFamilyParam {
        ...
        set => RaiseAndSetIfChanged(ref _selectedFamilyParam, value);
    }

    public ObservableCollection<FamilyParamViewModel> GridSelectedFamilyParams { get; set; } = new();

    private IEnumerable<FamilyParamViewModel> GetFamilyParams() {
        return _revitRepository.GetFamilyParams()
            .Where(item => item.StorageType == StorageType.Integer
                           || item.StorageType == StorageType.Double
                           || item.StorageType == StorageType.String)
    ...
    private void SaveTable(object param) {
        ...
        saveFileDialog.FileName = Path.Combine(
    ...
    private bool CanSaveTable(object param) {
        var familyParam =
            SelectedFamilyParams.FirstOrDefault(item => !string.IsNullOrEmpty(item.FamilyParamValues.GetValueErrors()));
        if(familyParam != null) {
    ...
    private void UpFamilyParam(object param) {
        var sortedParams = GridSelectedFamilyParams
            .Select(item => new {
                Param = item,
                Index = SelectedFamilyParams.IndexOf(item)
            })
