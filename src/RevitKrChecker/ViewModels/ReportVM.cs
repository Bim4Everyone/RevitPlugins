...
internal class ReportVM : BaseViewModel {
    ...
    private List<Element> _elems = [];

    private string _selectedErrorTooltips; 

    private ObservableCollection<ReportItemVM> _reportResult = [];
    ...
    public ObservableCollection<ReportItemVM> ReportResult {
        get => _reportResult;
        set => RaiseAndSetIfChanged(ref _reportResult, value);
    }

    public string SelectedErrorTooltips {
        get => _selectedErrorTooltips;
        set => RaiseAndSetIfChanged(ref _selectedErrorTooltips, value);
    }
    ...
    private void UpdateTooltips(object obj) {
        ...
        foreach(object item in selectedReportItemVMs) {
            var reportItemVM = item as ReportItemVM;
    ...
    private void Reсheck() {
        ...
        foreach(var element in tempElems) {
            foreach(var check in _nonStoppingChecks) {
