...
internal static class LineExtension {
    ...
    public static XYZ GetIntersectionWithFace(this Line line, Face face) {
        face.Intersect(line, out var result);
    ...
    public static XYZ GetPointFromLineEquation(this Line mepLine, Line wallLine) {
        ...
        double z = Math.Abs(mepLine.Direction.X) > 0.0001
            ? (xy.X - mepLine.GetEndPoint(0).X) / mepLine.Direction.X * mepLine.Direction.Z + mepLine.GetEndPoint(0).Z
            : (xy.Y - mepLine.GetEndPoint(0).Y) / mepLine.Direction.Y * mepLine.Direction.Z + mepLine.GetEndPoint(0).Z;
    ...
    private static XYZ GetHorizontalProjectionIntersection(this Line mepLine, Line wallLine) {
        ...
        projectedMepLine.Intersect(projectedWallLIne, out var result);
    ...
    public static XYZ GetIntersectionWithFaceFromEquation(this Line line, Face face) {
        ...
        double a = faceNormal.X;
        double b = faceNormal.Y;
        double c = faceNormal.Z;

        var pointOnFace = face.Evaluate(new UV(0, 0));

        double d = -(a * pointOnFace.X + b * pointOnFace.Y + c * pointOnFace.Z);

        //уравнение прямой в пространстве (x-x0)/p1 = (y-y0)/p2 = (z-z0)/p3
        var pointOnLine = line.GetEndPoint(0);
        double x0 = pointOnLine.X;
        double y0 = pointOnLine.Y;
        double z0 = pointOnLine.Z;

        double p1 = line.Direction.X;
        double p2 = line.Direction.Y;
        double p3 = line.Direction.Z;

        double x = (x0 * (p2 * b + p1 * p3 * c) - p1 * (d + b * y0 + z0 * c)) / (p1 * p3 * c + b * p2 + a * p1);
        double y;
        double z;
    ...
    public static XYZ GetIntersectionWithPlane(this Line line, Plane plane) {
        ...
        line.Intersect(projectedLine, out var result);
