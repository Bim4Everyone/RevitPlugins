...
internal static class HostObjectExtension {
    ...
    public static Solid GetHostElementOriginalSolid(this HostObject hostObject) {
        try {
            var loops = GetHostBoundCurveLoops(hostObject);

            var opts = new SolidOptions(ElementId.InvalidElementId, ElementId.InvalidElementId);
            Solid solid = default;
            foreach(var loopsPair in loops) {
                var currentSolid = GeometryCreationUtilities.CreateLoftGeometry(loopsPair, opts);
                if(solid is null) {
                    solid = currentSolid;
                } else {
    ...
    private static IList<IList<CurveLoop>> GetHostBoundCurveLoops(this HostObject hostObject) {
        var faces = GetHostBoundFaces(hostObject);
        if(faces.Length != 2) { throw new InvalidOperationException(); }
        var firstLoops = GetOuterCurveLoops(faces[0]);
        var secondLoops = GetOuterCurveLoops(faces[1]);

        List<List<CurveLoop>> loops = [];
    ...
    public static CurveLoop SimplifyCurveLoop(CurveLoop curveLoopRaw) {
        var simplifiedCurves = new CurveLoop();
        var curveLoopList = curveLoopRaw.ToList();

        Curve curvePrevious = null;
        for(int i = 0; i < curveLoopList.Count; i++) {
            var curveCurrent = curveLoopList[i];
    ...
    private static IList<CurveLoop> GetOuterCurveLoops(Face face) {
        ...
        for(int i = 0; i < curveLoops.Count; i++) {
            var curveLoopCurrent = curveLoops[i];
            var curve = curveLoopCurrent.First();
            var curveStart = curve.GetEndPoint(0);
            var curveEnd = curve.GetEndPoint(1);
            var curveDirection = (curveEnd - curveStart).Normalize();
    ...
    private static Face[] GetHostBoundFaces(this HostObject hostObject) {
        ...
        return IsVerticallyCompound(hostObject)
            ? hostObject.GetSideFaces().ToArray()
            : (new Face[] {
