...
internal static class MepCurveExtension {
    ...
    public static Level GetLevel(this MEPCurve curve) {
        ...
        return levelId.IsNull() ? null : (Level) curve.Document.GetElement(levelId);
    }
    ...
    private static double GetOffset(this MEPCurve curve) {
        ...
        double bottomElevation = curve.GetBottomElevation();
        double topElevation = curve.GetTopElevation();
        double center = curve.GetParamValueOrDefault<double>(BuiltInParameter.RBS_OFFSET_PARAM);
    ...
    private static XYZ FixElevation(XYZ xyz, IEnumerable<double> elevations) {
        double elevation = elevations.FirstOrDefault(item => Math.Abs(item - xyz.Z) == elevations.Min(e => Math.Abs(e - xyz.Z)));
