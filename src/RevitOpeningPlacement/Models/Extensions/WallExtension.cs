...
internal static class WallExtension {
    ...
    private static XYZ GetCentralPoint(Wall wall) {
        ...
        var bbCenter = bb.Min + (bb.Max - bb.Min) / 2;
        ...
        var exteriorFace = wall.GetGeometryObjectFromReference(
            HostObjectUtils.GetSideFaces(wall, ShellLayerType.Exterior)[0]) as PlanarFace;
        XYZ alignVector;
        if(exteriorFace != null) {

            var exteriorPlane = Plane.CreateByNormalAndOrigin(exteriorFace.FaceNormal, exteriorFace.Origin);

            var bbCenterProjection = exteriorPlane.ProjectPoint(bbCenter);
            var bbCenterToProjectionVector = bbCenterProjection - bbCenter;
