...
internal class SizeGetter {
    ...
    /// (равное половине значения размера, например, диаметра) в заданных направлениях
    ...
    /// </summary>
    /// <param name="directions">Векторы смещения осевой линии элемента ВИС в координатах файла стены</param>
    /// <param name="distance">Половина размера габарита, который надо спроецировать</param>
    /// <returns>Значение спроецированного габарита</returns>
    ...
    private double GetSizeFromIntersection(Line mepCurve, IEnumerable<XYZ> directions, double distance) {
        ...
        var pointPairs = GetPoints(results.Select(_plane.ProjectPoint).ToList());
        double maxDistance = pointPairs.Max(pp => pp.Point1.DistanceTo(pp.Point2));
