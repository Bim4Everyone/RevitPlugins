...
internal class FittingWallPointFinder : RoundValueGetter, IPointFinder {
    ...
    public XYZ GetPoint() {
        ...
        double z = solid.GetOutline().MinimumPoint.Z;
        ...
        var pointProjectedOnWallCenter = ProjectPointOntoWallCenter(_clash, clashCenter);
    ...
    private XYZ ProjectPointOntoWallCenter(FittingClash<Wall> fittingWallClash, XYZ point) {
        ...
        var wallCurve = (fittingWallClash.Element2.Location as LocationCurve).Curve;
        var startPoint = fittingWallClash.Element2Transform.OfPoint(wallCurve.GetEndPoint(0));
        var endPoint = fittingWallClash.Element2Transform.OfPoint(wallCurve.GetEndPoint(1));
        var startToThePointVector = point - startPoint;
