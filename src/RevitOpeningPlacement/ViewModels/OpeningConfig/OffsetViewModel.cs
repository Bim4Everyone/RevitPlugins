...
internal class OffsetViewModel : BaseViewModel {
    ...
    public string GetErrorText() {
        ...
        return From > To
            ? "значение параметра \"От\" должно быть меньше значения параметра \"До\"."
            : Offset > To ? "значение \"Зазора\" должно быть меньше значения параметра \"До\"." : null;
    }
    ...
    public string GetIntersectText(OffsetViewModel offset) {
        return (offset.From >= From || offset.To >= From) && (offset.To <= To || offset.From <= To)
            ? $"пересекаются диапазоны значений параметров \"{From}\" - \"{To}\" с \"{offset.From}\" - \"{offset.To}\"."
            : null;
    }
