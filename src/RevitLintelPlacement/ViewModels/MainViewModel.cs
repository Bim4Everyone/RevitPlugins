...
internal class MainViewModel : BaseViewModel {
    ...
    public GroupedRuleCollectionViewModel GroupedRules {
        get => _groupedRules;
        set => RaiseAndSetIfChanged(ref _groupedRules, value);
    }
    ...
    public ElementInfosViewModel ElementInfos {
        ...
        set => RaiseAndSetIfChanged(ref _elementInfosViewModel, value);
    }

    public List<LinkViewModel> SelectedLinks { get; set; } = new();

    public List<SampleModeViewModel> SampleModes { get; set; }

    public SampleModeViewModel SelectedSampleMode {
        get => _selectedSampleMode;
        set => RaiseAndSetIfChanged(ref _selectedSampleMode, value);
    }
    ...
    private void InitializeLinks() {
        ...
        if(links.Count > 0) {
            Links = new ObservableCollection<LinkViewModel>(
                links.Select(l => new LinkViewModel { Name = Path.GetFileNameWithoutExtension(l.Name) }));
        } else {
            Links = new ObservableCollection<LinkViewModel>();
        }
    }
    ...
    private void ApplySettings() {
        ...
        if(settings != null) {
            SelectedSampleMode = SampleModes.FirstOrDefault(item => item.Name.Equals(
                                     settings.SelectedModeNameRules,
                                     StringComparison.CurrentCulture))
                                 ?? SampleModes.FirstOrDefault();
            if(settings.SelectedLinks != null
               && settings.SelectedLinks.Count > 0) {
                SelectedLinks = Links.Where(item =>
                        settings.SelectedLinks.Any(sl => sl.Equals(
                            item.Name,
    ...
    private void PlaceLintels(object p) {
        ...
        var elementsInWalls = GetElementsInWalls(oldLintels);
        ...
        // отчет об ошибках
        if(ElementInfos.ElementInfos != null
           && ElementInfos.ElementInfos.Count > 0) {
            ShowReport();
        }
    }
    ...
    private ICollection<FamilyInstance> GetElementsInWalls(ICollection<LintelInfoViewModel> oldLintels) {
        var wallTypeNames = GroupedRules.SelectedRule.Rules
            .SelectMany(item => item.WallTypes.WallTypes.Where(w => w.IsChecked).Select(w => w.Name))
            .ToList();
    ...
    private View3D GetView() {
        ...
        using(var t = _revitRepository.StartTransaction("Подготовка к расстановке перемычек")) {
    ...
    private void CheckOldLintels(ICollection<string> links, ICollection<LintelInfoViewModel> oldLintels) {
        var lc = new LintelChecker(_revitRepository, GroupedRules, links, ElementInfos);

        using(var t = _revitRepository.StartTransaction("Проверка расставленных перемычек")) {
            lc.Check(oldLintels);
            t.Commit();
        }
    }

    private IEnumerable<LintelInfoViewModel> GetPlacedLintels(
        IEnumerable<FamilyInstance> elementInWalls,
        View3D view3D,
        ...
        using(var pb = GetPlatformService<IProgressDialogService>()) {
            ...
            using(var t = _revitRepository.StartTransaction("Расстановка перемычек")) {
    ...
    private IEnumerable<LintelInfoViewModel> PlaceLintels(
        IEnumerable<FamilyInstance> elementInWalls,
        View3D view3D,
        IEnumerable<string> links,
        ...
        foreach(var elementInWall in elementInWalls) {
            ...
            int elementInWallFixation = elementInWall.GetParamValueOrDefault(
                _revitRepository.LintelsCommonConfig.OpeningFixation,
                0);
            int value = elementInWallFixation;
            if(value == 1) {
                continue;
            }
            ...
            if(lintelType == null) {
                TaskDialog.Show(
                    "Предупреждение!",
                    $"В проект не загружено семейство с выбранным типоразмером \"{rule.SelectedLintelType}\".");
                yield break;
            }
            ...
            double offset = rule.LintelRightOffsetParameter.RightOffsetInternal;

            if(_revitRepository.DoesLeftCornerNeeded(
                   view3D,
                   elementInWall,
                   links,
                   ElementInfos,
                ...
                lintel.SetParamValue(
                    _revitRepository.LintelsCommonConfig.LintelLeftOffset,
                    leftOffset > 0 && leftOffset <= offset ? leftOffset : offset);
                lintel.SetParamValue(_revitRepository.LintelsCommonConfig.LintelLeftCorner, 0);
            }

            if(_revitRepository.DoesRightCornerNeeded(
                   view3D,
                   elementInWall,
                   links,
                   ElementInfos,
                ...
                lintel.SetParamValue(
                    _revitRepository.LintelsCommonConfig.LintelRightOffset,
                    rightOffset > 0 && rightOffset <= offset ? rightOffset : offset);
            }
        ...
        if(elevation == null) {
            ElementInfos.ElementInfos.Add(
                new ElementInfoViewModel(
                    ...
                    InfoElement.LackOfView.FormatMessage(viewName)));
            ShowReport();
            return false;
        }
    ...
    private void LockPlacedLintels(
        View3D view3D,
        View elevation,
        View plan,
        IEnumerable<LintelInfoViewModel> lintels) {
        using(var t = _revitRepository.StartTransaction("Закрепление перемычек")) {
