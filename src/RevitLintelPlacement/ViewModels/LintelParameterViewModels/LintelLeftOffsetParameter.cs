...
internal class LintelLeftOffsetParameter : BaseViewModel, ILintelParameterViewModel {
    ...
    public double LeftOffset {
        get => _leftOffset;
        set => RaiseAndSetIfChanged(ref _leftOffset, value);
    }

    public void SetTo(FamilyInstance lintel, FamilyInstance elementInWall) {
#if REVIT_2020_OR_LESS
            var value =
 UnitUtils.Convert(LeftOffset, DisplayUnitType.DUT_MILLIMETERS, DisplayUnitType.DUT_DECIMAL_FEET);
#elif REVIT_2021
            var value = UnitUtils.Convert(LeftOffset, UnitTypeId.Millimeters, UnitTypeId.Feet);
#else
        double value = UnitUtils.Convert(LeftOffset, UnitTypeId.Millimeters, UnitTypeId.Feet);
#endif
