...
internal class GroupedRuleCollectionViewModel : BaseViewModel {
    ...
    public GroupedRuleCollectionViewModel(RevitRepository revitRepository, ElementInfosViewModel elementInfos) {
        ...
        SelectedRule = Rules.FirstOrDefault(item => item.Name.Equals(
            _revitRepository.LintelsConfig.GetSettings(_revitRepository.GetDocumentName())?.SelectedPath,
    ...
    private RulesViewModel GetTemplateRules() {
        string templatePath = _revitRepository.GetTemplatePath();
    ...
    private RulesViewModel GetProjectRules() {
        string projectPath = _revitRepository.GetProjectPath();
        ...
        if(RevitRepository.HasEmptyProjectPath()) {
    ...
    private IEnumerable<RulesViewModel> GetLocalRules() {
        string projectPath = RevitRepository.LocalRulePath;
        if(projectPath != null
           && Directory.Exists(projectPath)) {
            string[] configPaths = Directory.GetFiles(projectPath, "*.json");
    ...
    private void Load(object p) {
        ...
        SelectedRule =
            Rules.FirstOrDefault(item => item.Name.Equals(newRules.Name, StringComparison.CurrentCulture));
    }
    ...
    private bool CanDelete(object p) {
        return SelectedRule != null && SelectedRule.RuleConfigManager.CanDelete;
    }
}
