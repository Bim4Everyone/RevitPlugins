...
internal class RulesViewModel : BaseViewModel, INamedEntity {
    ...
    public RuleConfigManager RuleConfigManager {
        ...
        set => RaiseAndSetIfChanged(ref _ruleConfigManager, value);
    }

    public ObservableCollection<GroupedRuleViewModel> Rules {
        get => _rules;
        set => RaiseAndSetIfChanged(ref _rules, value);
    }
    ...
    public static RulesViewModel GetLocalRules(
        RevitRepository revitRepository,
        ElementInfosViewModel elementInfos,
        RuleConfig ruleConfig) {
    ...
    public static RulesViewModel GetProjectRules(
        RevitRepository revitRepository,
        ElementInfosViewModel elementInfos,
        RuleConfig ruleConfig) {
        return new RulesViewModel(revitRepository, elementInfos, ruleConfig) {
    ...
    public static RulesViewModel GetTemplateRules(
        RevitRepository revitRepository,
        ElementInfosViewModel elementInfos,
        RuleConfig ruleConfig) {
        return new RulesViewModel(revitRepository, elementInfos, ruleConfig) {
    ...
    public static RulesViewModel GetLoadedRules(
        RevitRepository revitRepository,
        ElementInfosViewModel elementInfos,
        RuleConfig ruleConfig) {
        return new RulesViewModel(revitRepository, elementInfos, ruleConfig) {
    ...
    public string GetErrorText() {
        for(int i = 0; i < Rules.Count - 1; i++) {
            for(int j = i + 1; j < Rules.Count; j++) {
                var commonWallTypes = Rules[i]
                    .WallTypes.WallTypes.Where(item => item.IsChecked)
    ...
    private void InitializeRules() {
        if(_ruleConfig.RuleSettings == null
           || _ruleConfig.RuleSettings.Count == 0) {
    ...
    private RuleConfig GetUpdatedConfig() {
        _ruleConfig.ProjectConfigPath = Path.Combine(
            Path.GetDirectoryName(_ruleConfig.ProjectConfigPath),
            Name + ".json");
    ...
    private void Rename(object p) {
        var newRulesName = new RulesNameViewModel(
            ((ObservableCollection<RulesViewModel>) p).Select(item => item.Name),
            Name);
        var view = new RulesNameView {
            DataContext = newRulesName,
            Owner = p as Window
        };
    ...
    private bool CanSave(object p) {
        return RuleConfigManager.CanSave && string.IsNullOrEmpty(GetErrorText());
    }

    private bool CanRemoveGroupedRule(object p) {
        return p is GroupedRuleViewModel;
    }

    private bool CanRename(object p) {
        return RuleConfigManager.CanRename && p is ObservableCollection<RulesViewModel>;
    }
}
