...
internal class LintelsSettings : ProjectSettings {
    ...
    public List<string> SelectedLinks { get; set; } = new();
}
...
internal class LintelsCommonConfig {
    ...
    public List<string> ReinforcedConcreteFilter { get; set; } = new();
    ...
    [JsonIgnore]
    public Dictionary<string, StorageType> ParamterType { get; set; } = new() {
    ...
    public static LintelsCommonConfig GetLintelsCommonConfig(string documentName) {
        if(File.Exists(GetDocumentConfigPath(documentName))) {
            return JsonConvert.DeserializeObject<LintelsCommonConfig>(
                File.ReadAllText(GetDocumentConfigPath(documentName)));
        }
    ...
    private static string GetLintelsCommonConfigPath() {
        string projectConfigPath = RevitRepository.ProfilePath;
        string pluginName = nameof(RevitLintelPlacement);
        string revitVersion = string.IsNullOrEmpty(ModuleEnvironment.RevitVersion)
            ? "2020"
