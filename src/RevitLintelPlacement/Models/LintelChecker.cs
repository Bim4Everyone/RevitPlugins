...
internal class LintelChecker {
    ...
    public LintelChecker(
        RevitRepository revitRepository,
        GroupedRuleCollectionViewModel groupedRules,
...
internal class LintelGroupChecker : IChecker {
    public IResultHandler Check(FamilyInstance lintel, FamilyInstance elementInWall) {
        if(lintel.GroupId != ElementId.InvalidElementId
           || lintel.SuperComponent != null) {
    ...
    public IResultHandler Check(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        if((int) lintel.GetParamValue(_revitRepository.LintelsCommonConfig.LintelFixation) == 1) {
            _elementInfos.ElementInfos.Add(
                new ElementInfoViewModel(lintel.Id, InfoElement.LintelIsFixedWithoutElement) {
                    Name = lintel.Name,
...
internal class LintelRuleChecker : IChecker {
    ...
    public IResultHandler Check(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        if(lintel.GetParamValueOrDefault(_revitRepository.LintelsCommonConfig.LintelFixation, 0) == 1) {
            return new EmptyResult { Code = ResultCode.Correct };
        }
        ...
        return new WrongLintelParameters(
            _revitRepository,
            results,
            rule,
            lintel,
            elementInWall,
            rightOffset,
            leftOffset) { Code = ResultCode.Correct };
    }
    ...
    private ParameterCheckResult CheckLintelThickness(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        double lintelThickness =
            (double) lintel.GetParamValueOrDefault(_revitRepository.LintelsCommonConfig.LintelThickness);
    ...
    private ParameterCheckResult CheckLintelWidth(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        double lintelWidth =
            (double) lintel.GetParamValueOrDefault(_revitRepository.LintelsCommonConfig.LintelWidth);
    ...
    private ParameterCheckResult CheckLintelLeftOffset(
        FamilyInstance lintel,
        FamilyInstance elementInWall,
        ConcreteRuleViewModel rule,
        out double offset) {
        double lintelLeftOffset =
            lintel.GetParamValueOrDefault<double>(_revitRepository.LintelsCommonConfig.LintelLeftOffset);
        bool lintelCorner =
            lintel.GetParamValueOrDefault<int>(_revitRepository.LintelsCommonConfig.LintelLeftCorner) == 1
                ? true
                : false;
        if(_revitRepository.DoesLeftCornerNeeded(
               ...
               _linkNames,
               _elementInfos,
               rule.LintelRightOffsetParameter.RightOffsetInternal,
               out offset)) {
    ...
    private ParameterCheckResult CheckLintelRightOffset(
        FamilyInstance lintel,
        FamilyInstance elementInWall,
        ConcreteRuleViewModel rule,
        out double offset) {
        double lintelRightOffset =
            lintel.GetParamValueOrDefault<double>(_revitRepository.LintelsCommonConfig.LintelRightOffset);
        bool lintelCorner =
            lintel.GetParamValueOrDefault<int>(_revitRepository.LintelsCommonConfig.LintelRightCorner) == 1
                ? true
                : false;
...
internal class GeometricalLintelChecker : IChecker {
    ...
    public IResultHandler Check(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        double height = lintel.LevelId.IsNotNull()
            ? ((Level) _revitRepository.GetElementById(lintel.LevelId)).Elevation
            : 0;
        ...
        _elementInfos.ElementInfos.Add(
            new ElementInfoViewModel(lintel.Id, InfoElement.LintelGeometricalDisplaced) {
                Name = lintel.Name,
                LevelName = lintel.LevelId != null ? _revitRepository.GetElementById(lintel.LevelId)?.Name : null
            });
