...
internal class RevitRepository {
    ...
    private readonly BuiltInParameter[] _bottomBarHeightsParams = {
    ...
    public static string LocalRulePath =>
        Path.Combine(
    ...
    public string GetProjectPath() {
        return Path.Combine(
            ...
            nameof(RevitLintelPlacement),
            "Rules",
            GetDocumentName() + ".json");
    }

    public static bool HasEmptyProjectPath() {
        return Directory.Exists(_settingsPath);
    }

    public string GetDocumentName() {
        string documentName = string.IsNullOrEmpty(_document.Title)
            ? "Без имени"
    ...
    public IEnumerable<FamilySymbol> GetLintelTypes() {
        return new FilteredElementCollector(_document)
            ...
            .Cast<FamilySymbol>()
            .Where(e => LintelsCommonConfig.LintelFamily.Equals(
                e.Family?.Name,
    ...
    public View3D GetView3D() {
        ...
        if(view == null) {
            using(var transaction = _document.StartTransaction("Создание 3D-вида")) {
                view = CreateView3d();
    ...
    private void UpdatePhase(View3D view) {
        var activeView = _uiDocument.ActiveGraphicalView;
        view.SetParamValue(
    ...
    private View3D CreateView3d() {
        ...
        var view = View3D.CreateIsometric(_document, type.Id);
    ...
    public FamilyInstance PlaceLintel(FamilySymbol lintelType, FamilyInstance elementInWall) {
        ...
        var center = GetLocationPoint(elementInWall);
        FamilyInstance lintel = null;
        if(elementInWall.LevelId != null
           && elementInWall.LevelId != ElementId.InvalidElementId) {
            lintel = _document.Create.NewFamilyInstance(
                center,
                lintelType,
                (Level) _document.GetElement(elementInWall.LevelId),
    ...
    public List<FamilyInstance> GetElementsInWall(
        ...
        var openings = collector1.OfClass(typeof(FamilyInstance))
            ...
            .Where(f => f?.Symbol != null
                        && f.Symbol?.Family != null
                        && f.Symbol.Family.Name != null
                        && f.Symbol.Family.Name.ToLower().Contains(LintelsCommonConfig.HolesFilter.ToLower()))
            .ToList();
        ...
        return openings.Union(wallsAndDoors, new FamilyInstanceComparer())
    ...
    public bool CheckUp(View3D view3D, FamilyInstance elementInWall, IEnumerable<string> linkNames) {
        ...
        var refWithContext =
            GetNearestWallOrColumn(
                view3D,
                elementInWall,
                new XYZ(viewPoint.X, viewPoint.Y, viewPoint.Z - 0.32),
                new XYZ(0, 0, 1),
                false); // чтобы точка точно была под гранью стены
        ...
        refWithContext = GetNearestWallOrColumn(
            view3D,
            elementInWall,
            new XYZ(viewPoint.X, viewPoint.Y, viewPoint.Z - 0.32),
            new XYZ(0, 0, 1),
        ...
        if(wallOrColumn.Category.Id == new ElementId(BuiltInCategory.OST_StructuralColumns)
           || wallOrColumn.Category.Id == new ElementId(BuiltInCategory.OST_StructuralFraming)) {
            return false;
        }

        if(wallOrColumn is RevitLinkInstance linkedInstance) {
            return !linkNames.Any(l => l.Equals(
                linkedInstance.GetLinkDocument().Title,
                StringComparison.CurrentCultureIgnoreCase));
        }
    ...
    public bool DoesRightCornerNeeded(
        View3D view3D,
        FamilyInstance elementInWall,
        IEnumerable<string> linkNames,
        ElementInfosViewModel elementInfos,
        double offset,
        out double realOffset) {
        return DoesCornerNeeded(
            view3D,
            elementInWall,
            new XYZ(-1, 0, 0),
            linkNames,
    ...
    public bool DoesLeftCornerNeeded(
        View3D view3D,
        FamilyInstance elementInWall,
        IEnumerable<string> linkNames,
        ElementInfosViewModel elementInfos,
    ...
    private bool DoesCornerNeeded(
        ...
        var refWithContext = GetNearestWallOrColumn(view3D, elementInWall, viewPoint, direction, true);
        ...
        object elementWidth = elementInWall.GetParamValueOrDefault(LintelsCommonConfig.OpeningWidth)
                              ?? elementInWall.Symbol.GetParamValueOrDefault(LintelsCommonConfig.OpeningWidth);

        if(elementWidth == null) {
            elementInfos.ElementInfos.Add(
        ...
        if((wallOrColumn is Wall wall && ContainsReinforce(wall.Name))
           || wallOrColumn.Category.Id == new ElementId(BuiltInCategory.OST_StructuralColumns)
           || wallOrColumn.Category.Id == new ElementId(BuiltInCategory.OST_StructuralFraming)
           || (wallOrColumn is RevitLinkInstance linkedInstance
               && linkNames.Any(l => l.Equals(
                   linkedInstance.GetLinkDocument().Title,
                   StringComparison.CurrentCultureIgnoreCase)))) {
        ...
        for(int i = 0; i < 2; i++) {
            _revitEventHandler.TransactAction = () => {
                ...
                if(!(commandId is null)
                   && _uiDocument.Application.CanPostCommand(commandId)) {
    ...
    public async Task MirrorLintel(FamilyInstance lintel, FamilyInstance elementInWall) {
        ...
        _revitEventHandler.TransactAction = () => {
            using(var t = _document.StartTransaction("Поворот перемычки")) {
                var ids = lintel
    ...
    private ReferenceWithContext GetNearestWallOrColumn(
        View3D view3D,
        FamilyInstance fi,
        XYZ viewPoint,
        XYZ direction,
        ...
        var logicalOrFilter = new LogicalOrFilter(
            new List<ElementFilter> {
                logicalAndFilter,
                categoryFilter,
                classFilter2
    ...
    public void LockLintel(
        ...
        if(leftRightElement.Count > 0
           && leftRightLintel.Count > 0) {
    ...
    public bool CheckElementInWallParameter(
        FamilyInstance elementInWall,
        ElementInfosViewModel elementInfos,
        IEnumerable<string> wallTypes = null) {
        if(!elementInWall.IsExistsParam(LintelsCommonConfig.OpeningHeight)
           ...
           && (wallTypes == null
               || wallTypes.Any(w => w.Equals(
                   elementInWall.Host.Name,
                   StringComparison.CurrentCultureIgnoreCase)))) {
            elementInfos.ElementInfos.Add(
                new ElementInfoViewModel(
                    elementInWall.Id,
    ...
    public XYZ GetLocationPoint(FamilyInstance elementInWall) {
        ...
        double? height = elementInWall.GetParamValueOrDefault<double?>(LintelsCommonConfig.OpeningHeight)
    ...
    public bool CheckLintelType(FamilySymbol lintelType, ElementInfosViewModel elementInfos) {
        ...
        try {
            ...
            foreach(string configParameter in configParameterNames) {
                if(!parameterNames.Any(p => p.Equals(configParameter, StringComparison.CurrentCultureIgnoreCase))) {
                    result = false;
                    elementInfos.ElementInfos.Add(
                        new ElementInfoViewModel(
                            lintelType.Id,
                            InfoElement.MissingLintelParameter.FormatMessage(configParameter)) {
    ...
    public IEnumerable<ParameterViewModel> GetParametersFromFamilies(string familyName) {
        var family = new FilteredElementCollector(_document)
            ...
            .FirstOrDefault(f =>
                ...
                && familyName.Equals(f.Name, StringComparison.CurrentCultureIgnoreCase));
    ...
    private void AddBlankParameterInfo(ElementInfosViewModel elementInfos, string parameterName) {
        string description = TypeDescriptor
        ...
        elementInfos.ElementInfos.Add(
            new ElementInfoViewModel(
                ...
                InfoElement.BlankParamter.FormatMessage(description)));
    }
}
