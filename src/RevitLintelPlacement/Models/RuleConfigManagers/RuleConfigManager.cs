...
internal class RuleConfigManager : IRuleConfigManager {
    ...
    public RuleConfig Copy(RuleConfig config, IEnumerable<RuleConfig> configs) {
        ...
        string name = Renamer.GetName(config, configs);
        string directory = RevitRepository.LocalRulePath;
    ...
    public void Delete(RuleConfig config) {
        if(File.Exists(config.ProjectConfigPath)
           && config.ProjectConfigPath.EndsWith(".json", StringComparison.CurrentCultureIgnoreCase)) {
    ...
    public void Rename(RuleConfig config, string name) {
        if(File.Exists(config.ProjectConfigPath)) {
            File.Move(
                ...
                Path.Combine(Path.GetDirectoryName(config.ProjectConfigPath), name + ".json"));
        }
    }
