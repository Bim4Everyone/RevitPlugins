...
internal class SpecificationService {
    ...
    public ViewSchedule DuplicateSpec(ViewSchedule viewSchedule, string newViewSpecName) {
        var newViewSpec = _revitRepository.GetSpecByName(newViewSpecName);
    ...
    private void ChangeSpecFilter(ViewSchedule spec, string specFilterName, Action<ScheduleFilter> action) {
        var specificationDefinition = spec.Definition;
        var specificationFilters = specificationDefinition.GetFilters().ToList();
        var newScheduleFilters = new List<ScheduleFilter>();

        for(int i = 0; i < specificationFilters.Count; i++) {
            var currentFilter = specificationFilters[i];
            var scheduleFieldFromFilter = specificationDefinition.GetField(currentFilter.FieldId);
