...
internal static class Delimiter {
    ...
    public static SplittedViewName SplitViewName(string originalName, SplitViewOptions splitViewOptions) {
        ...
        return new SplittedViewName {
            Prefix = Removes(prefix),
            ViewName = Removes(viewName),
            Suffix = Removes(suffix),
            Elevations = Removes(elevation)
        };
    }

    public static string CreateViewName(SplittedViewName splittedViewName) {
        string[] values = [
            splittedViewName.Prefix, splittedViewName.ViewName, splittedViewName.Suffix, splittedViewName.Elevations
        ];
        return string.Join(new string([_value]), values.Where(item => !string.IsNullOrEmpty(item)));
    }
    ...
    private static string ReplaceName(string originalName, params string[] values) {
        originalName = values
            .Where(item => !string.IsNullOrEmpty(item))
            .Aggregate(originalName, (current, value) => current.Replace(value, string.Empty));

        return originalName.Trim().Trim(_value);
    }
...
internal class SplittedViewName {
    ...
    public bool HasElevation => !string.IsNullOrEmpty(Elevations);
