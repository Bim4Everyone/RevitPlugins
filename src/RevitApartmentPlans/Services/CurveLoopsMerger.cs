...
internal class CurveLoopsMerger : ICurveLoopsMerger {
    ...
    public CurveLoop Merge(ICollection<CurveLoop> curveLoops) {
        double offset = 0;
        var curveLoopsXOY = CreateCopyLoopsOnXOY(curveLoops);
        while(offset <= _offsetMax) {
            var offsettedLoops = CreateOffsetLoop(curveLoopsXOY, offset);
            var solids = CreateSolids(offsettedLoops);
            ...
            if(unitedSolids.Length == 1) {
                ...
                var bottomFace = GetBottomFace(mergedSolid);
                var bottomLoop = GetOuterCurveLoop(bottomFace);
                var transform = GetVerticalTransformFromZero(curveLoops.First());
                try {
    ...
    private Solid CreateSolid(CurveLoop bottomLoop) {
        try {
            var topLoop = CurveLoop.CreateViaTransform(bottomLoop, Transform.CreateTranslation(new XYZ(0, 0, 1)));
            return GeometryCreationUtilities.CreateLoftGeometry(new CurveLoop[] { bottomLoop, topLoop }, _solidOptions);
        } catch(Autodesk.Revit.Exceptions.ApplicationException) {
            var bottomRectangleLoop = _rectangleLoopProvider.CreateRectCounterClockwise(bottomLoop);
            var topRectangleLoop = CurveLoop.CreateViaTransform(
