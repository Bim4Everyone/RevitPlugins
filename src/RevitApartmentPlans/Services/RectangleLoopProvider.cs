...
internal class RectangleLoopProvider : IRectangleLoopProvider {
    ...
    public CurveLoop CreateRectCounterClockwise(ICollection<CurveLoop> curveLoops) {
        var points = curveLoops
            ...
            .ToArray();
        var outline = new Outline(points[0], points[1]);
        ...
        var leftBottom = new XYZ(minX, minY, z);
        var leftTop = new XYZ(minX, maxY, z);
        var rightTop = new XYZ(maxX, maxY, z);
        var rightBottom = new XYZ(maxX, minY, z);
