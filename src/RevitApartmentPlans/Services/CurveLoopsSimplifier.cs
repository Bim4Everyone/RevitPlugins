...
internal class CurveLoopsSimplifier : ICurveLoopsSimplifier {
    ...
    private bool IsContinuation(Curve first, Curve second) {
        return first is not null && second is not null && (first is Line firstLine) && (second is Line secondLine) && firstLine.Direction.IsAlmostEqualTo(secondLine.Direction)
