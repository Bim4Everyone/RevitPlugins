...
* В новых плагинах включен `<ImplicitUsings>enable</ImplicitUsings>`, поэтому пространства имен
  `System`, `System.Collections.Generic`, `System.IO`, `System.Linq`, `System.Threading`
  и `System.Threading.Tasks` подключать директивой `using` не нужно.
  Правила порядка и группировки выше действуют для всех остальных пространств имен.
...
### Правила стиля в `xaml`

*   Каждое свойство элемента управления на отдельной строке.
*   Свойства сортируются по типу от простого к сложному.
    Сложность типа определяется уровнем наследования от базового класса `object`.
...
* Не добавляйте `using` для пространств имен, которые уже подключены через `ImplicitUsings`.
  Если пространство имен требуется во всем проекте, объявляйте его один раз
  через `<Using Include="..."/>` в `.csproj`, а не в каждом файле.
...
* В плагинах включен `<Nullable>enable</Nullable>`. Тип, который может быть null,
  помечается `?`; отсутствие `?` — это гарантия того, что значение не null.
    * Оператор `!` (null-forgiving) допустим только там, где отсутствие null доказуемо,
      и не как способ заглушить предупреждение компилятора.
    * Nullable-диагностики компилятора (CS86xx) превращаются в ошибки
      из-за `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>`,
      поэтому новый плагин обязан быть nullable-чистым.
* Для обработки значений, которые действительно могут быть null, используйте:
