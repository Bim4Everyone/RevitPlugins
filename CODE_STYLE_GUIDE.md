...
*   Не приветствуется:
    ...
    *   При работе с встроенными числовыми переменными, которые определяются компилятором, например, `var
        number = 12 * ReturnsFloat();`
    *   Когда пользователи точно получат пользу от того, что знают тип, например, `var
        listOfItems = GetList();`
